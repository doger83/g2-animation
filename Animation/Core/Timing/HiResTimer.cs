...
public class HiResTimer
{
    ...
    /// </value>
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="value"/> is negative or <see cref="float.NaN"/>.</exception>
    /// <remarks>
    ...
    public float IgnoreElapsedThreshold
    {
        ...
        set
        {
            if (value <= 0f || float.IsNaN(value))
            {
                throw new ArgumentOutOfRangeException(nameof(IgnoreElapsedThreshold));
            }
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="interval"/> is negative or <see cref="float.NaN"/>.</exception>
    public HiResTimer(float interval)
    {
        if (interval < 0f || float.IsNaN(interval))
        {
            throw new ArgumentOutOfRangeException(nameof(interval));
        }
    ...
    public void Start()
    {
        ...
        isRunning = true;
        Thread thread = new(ExecuteTimer) { Priority = ThreadPriority.Highest };
        thread.Start();
    }
